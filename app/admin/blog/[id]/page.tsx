...
export default function BlogEditorPage() {
  ...
  // Auto-generate slug only when title changes and slug wasn't manually edited
  useEffect(() => {
    if (!slugManuallyEdited && post === null) {
  ...
  if (loading) {
    return (
      <div className="admin-content" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 200 }}>
        <span style={{ color: 'var(--text-secondary)', fontSize: 14 }}>Loading post...</span>
      </div>
    );
  }
  ...
  if (error && !post) {
    return (
      <div className="admin-content">
        ...
        <Link href="/admin/blog" className="admin-btn admin-btn-ghost" style={{ marginTop: 12 }}>← Back to Blog</Link>
      </div>
    );
  }
  ...
  return (
    <div className="admin-content">
      <div className="admin-breadcrumb">
        ...
        <span>/</span>
        <span className="current">{post?.title || 'Edit Post'}</span>
      </div>
        ...
        <div>
          ...
          {post && (
            <p className="admin-page-subtitle">
              Created {new Date(post.createdAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}
              {post.publishedAt && ` · Published ${new Date(post.publishedAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}`}
            </p>
          )}
        </div>
        <button
